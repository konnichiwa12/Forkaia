...
function mygamesload(){
		let num = window.localStorage.getItem("!!!!numOfGames");
		for (let i = 1; i <= num; i++){
			let currKey = window.localStorage.key(i);
			let currGame = JSON.parse(window.localStorage.getItem(currKey));
			document.getElementById(i + 'name').innerHTML = currGame.name;
			document.getElementById(i + 'address').innerHTML = currGame.address;
			document.getElementById(i + 'city').innerHTML = currGame.city + ' ' + currGame.zip;
	...
	function updategame(){
		let num = document.getElementById('gamenum').value;
		let oldver = JSON.parse(window.localStorage.getItem(window.localStorage.key(num)));
		console.log(oldver);
		let oldid;
		if (ifempty("name") == false){
			oldver.name = document.getElementById("name").value;
		}
		if (ifempty("ad") == false){
			oldver.name = document.getElementById("ad").value;
		}
		if (ifempty("city") == false){
			oldver.city = document.getElementById("city").value;
		}
		if (ifempty("id") == false){
			oldid = oldver.id;
			oldver.id = document.getElementById("id").value;
		}
		if (ifempty("max") == false){
			oldver.maxp = document.getElementsByClassName("max");
		}
		if (ifempty("info") == false){
			oldver.info = document.getElementsByClassName("info");
		}
		window.localStorage.setItem(oldid, JSON.stringify(oldver));
		mygamesload();
	}
