...
//alert user that you are leaving a page
...
//for testing purposes, same as above method
...
//for changing button colors when you press buttons in CreateGames.html
...
//for changing button colors and changing values when pressing buttons in CreateGames.html and SearchGames.html
function enable(target1, target2){//
...
function disable(target1, target2){//
...
//for changing button appearance and changing values when pressing buttons in SearchGames.html
function check(check, uncheck, checklist, unchecklist){
...
//alert for confirming input information in SearchGames.html
...
//change appearance of window in SearchGames.html
...
//check and send error messages of input in SearchGames.html
...
//Used to put in date/time information
function UploadTime(){
	let today = new Date();
...
//check input and report errors CreateGames.html
...
//Change appearance of window and update LocalStorage values of CreateGames.html, return true if successful
...
//check character length of a string (for debugging purposes)
...
//check if an html element is empty like textboxes (mainly for debugging)
function ifempty(id){
...
//after inputting data in edit tool, update info in LocalStorage
function updategame(){
		let num = document.getElementById('gamenum').value;
...
//load games when opening MyGames.html
function mygamesload(){
			//if localStorage is empty, create a temparray and numOfGames in LocalStorage and and create blanks in MyGames.html
			if (window.localStorage.getItem("numOfGames") == null){
				window.localStorage.setItem("numOfGames", 0);
				let temparray = ["empty", "empty", "empty", "empty", "empty"];
				window.localStorage.setItem('keyarray', JSON.stringify(temparray));
			...
			//just create blanks if temparray and numOfGames is already initialized
			...
			//if there are games registered, update MyGames.html
...
//load in the information about the game in ViewGame.html
...
//delete all information in LocalStorage
function clearall(){
	if(confirm("Are you sure?")){
...
//check which light mode is selected in Settings.html
function checklightingmode(){
	if (window.localStorage.getItem("lighting") == null){
		window.localStorage.setItem("lighting", 0);
		document.getElementById("Light").checked = true;
		return;
	}
	else if (JSON.parse(window.localStorage.getItem('lighting')) == 1){
		if (document.URL == "settings.html"){
			document.getElementById("Dark").checked = true;
		}
		document.getElementById("top").style.color = "#9e6700";
		document.getElementById("SM").style.color = "#9e6700";
		document.getElementById("Whole").style["background-color"] = "rgb(0, 0, 0, 0.3)";
	}
	else{
		if (document.URL == "settings.html"){
			document.getElementById("Light").checked = true;
		}
		document.getElementById("top").style.color = "#CE8500";
		document.getElementById("SM").style.color = "#CE8500";
		document.getElementById("Whole").style["background-color"] = "rgb(0, 0, 0, 0)";
	}
}

//change the lighting mode and update CSS
function changelighting(){
	//lightmode
	if (JSON.parse(window.localStorage.getItem('lighting') == 0)){
		document.getElementById("top").style.color = "#9e6700";
		document.getElementById("SM").style.color = "#9e6700";
		document.getElementById("Whole").style["background-color"] = "rgb(0, 0, 0, 0.3)";
		window.localStorage.setItem('lighting', JSON.stringify(1));
	}
	else{
		document.getElementById("top").style.color = "#CE8500";
		document.getElementById("SM").style.color = "#CE8500";
		document.getElementById("Whole").style["background-color"] = "rgb(0, 0, 0, 0)";
		window.localStorage.setItem('lighting', JSON.stringify(0));
	}
...
//constructor for Game object, contains all information about a game
